...
export class WebGLCanvasSystem extends System {
  ...
  updateSceneData(delta, worldState, canvasContext) {
    ...
    if (!scene._fontplay) {
      const glyphs = fonts.futural.stringToGlyphs("Hello, world!");
      const glyphShapes = [];
      let xCursor = 0;
      let yCursor = 0;
      for (const glyph of glyphs) {
        const { left, right } = glyph;
        xCursor += Math.abs(left);
        glyphShapes.push([]);
        for (const point of glyph.points) {
          if (!point) {
            ...
            const [ x, y ] = point;
            glyphShapes[glyphShapes.length - 1].push([ x + xCursor, y + yCursor ]);
          }
        }
        xCursor += Math.abs(right);
      }
