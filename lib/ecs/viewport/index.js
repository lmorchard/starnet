...
import * as WebGLViewport from "./webgl/index.js";

//const ActiveViewport = CanvasViewport;
const ActiveViewport = WebGLViewport;
...
export async function init(world) {
  ...
  await ActiveViewport.init(world);
}

export async function initState(worldState, props) {
  worldState
    .addComponent(Camera)
    .addComponent(Lerper)
  ...
  await ActiveViewport.initState(worldState, props);
}

export function draw(worldState, interpolationPercentage) {
  ActiveViewport.draw(worldState, interpolationPercentage);
}
