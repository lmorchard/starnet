...
export const genFlags = (seed, spec) => {
  ...
  return [ toRandom, toIndex, flags, (random) => flags(toIndex[random]) ];
};

export const Star = (seed) => {
  const { rng, base } = BaseNode({ seed });
  const [ toRandom, toIndex, flagsFromIndex, flagsFromRandom ] = genFlagsFactory(rng(), [
    [4, ['rich']],
    [2, ['datacenter']],
    [10, ['poor']],
    [8, ['farming']],
  ...
  for (let rnd of Object.values(toRandom)) {
    console.log("FLAGS", rnd, flagsFromRandom(rnd));
  }
