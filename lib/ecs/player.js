...
export class PlayerStateSystem extends System {
  ...
  updateSceneForContextNode(playerState, pendingContextNode) {
    ...
    const rng = mkrng(contextNode.addr);
    ...
    const skipAddr = playerState.pendingCurrentNode.addr;
    spawnNode(playerState.pendingCurrentNode);

    const spawn = () => {
      const { value, done } = sceneNodes.next();
      if (value) {
        const { node } = value;
        if (node.addr !== skipAddr) {
          spawnNode(node);
        }
      }
      if (!done) {
