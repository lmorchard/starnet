...
export function initUniverse() {
  ...
  console.log(nodeFmt(nodeByAddr('1234:fff9:2229:e1fc:2c11:f0db')));
}

const nodeFmt = ({ type, name, addr, flags } = {}, indent = ' ') =>
  `${indent}${addr} ${type}: ${name} ${flags}`;

const nodeByAddr = (addr) => {
  const parts = addr.split(':');
  const universe = Universe({ addr: parts[0] });

  let child = universe;
  for (let idx = 1; idx < parts.length; idx++) {
    const childAddr = parts.slice(0, idx).join(':')
    child = child.child();
    if (!child) {
      return undefined;
    }
