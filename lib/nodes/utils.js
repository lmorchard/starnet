  ...
  init({ childVariants, ...props }) {
    ...
    for (let [count, childClass] of childVariants) {
      ...
      for (let idx = 0; idx < count; idx++) {
        ...
        const className = childClass.constructor.name;
        if (!classNameToAddrs[className]) {
          classNameToAddrs[className] = [];
        }
        classNameToAddrs[className].push(childAddr);
      }
    }
