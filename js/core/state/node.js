...
/** Records the initial (max) coherence for a node — set once at auto-burn start for UI readouts. */
export function setNodeCoherenceMax(nodeId, value) {
  mutate((s) => {
    const node = s.nodes[nodeId];
    if (node) node.coherenceMax = value;
  });
}
