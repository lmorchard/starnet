...
export function initUniverse() {
  ...
  for (let i = 1; i < child.childAddrs.length; i++) {
    console.log(nodeFmt(child.childAt(i)));
  } 

}

export const nodeFmt = ({ type, name, addr, flags } = {}, indent = ' ') =>
...
export function Star({ addr }) {
  ...
  const { addrToFlags, flagToAddrs } = genFlags(addr, rng, spec);
