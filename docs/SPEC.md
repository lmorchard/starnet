...
## Experiments / prototypes
  ...
  - Network graph navigation
      - <https://lmorchard.github.io/sketches-v01/sketches/nodes/index.html>
  - Hacking based on perishable exploit collection vs varying system strengths
      - <https://lmorchard.github.io/sketches-v01/sketches/hackcombat/index.html>
...
https://twitter.com/HackersCurator/status/1519148791370117121
...
## Misc game loop ideas

Starnet core hacking game loop should be focused on LANs or contained sub-networks akin to Legend of Zelda dungeons.

Each node is a “room” where various actions can be executed. 

https://twitter.com/HackersCurator/status/1519148791370117121

Nodes may need to be attacked with exploits to bring them into a privilege escalation condition wherein player actions are enabled.

Exploits can be collectibles. I have a proof-of-concept mini-game encapsulating exploits vs system security level with conditions for success and exploit decay over time and failures.

Maybe node-specific player actions are also collectibles - i.e. player abilities with respect to specific node types are technique upgrades to be acquired. Then, a player progression path could be the accumulation of node-wise techniques that make more complex networks susceptible to solution.

Some actions reveal and grant connection to nodes deeper in the dungeon. 

Other actions may modify the function of the current node - e.g. redirect data flows, filter & modify event streams, trigger player alerts on conditions, skim funds into the player’s wallet.

Popping up a level, these networks can be mechanical systems composed of nodes providing functions to other nodes. Trying to think of influences like EVE Online planetary interaction, Factorio, Creeper World, etc.

For example:
a router can feed events to an audit log collector. 
audit log collector can be queried by a security monitor.
If the router is tampered with, it sends events to the audit log collector.
Security monitor detects patterns in audit log, raises an alert condition
Maybe some other system receives the alert condition and messages a human administrator beyond the dungeon
If the player can subvert the audit log collector or security monitor or alert messaging system, they will be able to hack away at the router without drawing human administrator attention

General idea is that there should be an arrangement of parts that the player can splice apart and rearrange so that they can safely accomplish the overall goal of the session. Each node is a thing to tinker with via exploits and actions, but the overall network is a puzzle.

I want to come up with other instances of this sort of circuit-like assemblage of nodes with gameplay opportunities for subversion and redirection. It would be really cool to eventually rig these up with procedural generation to spin up semi-unique puzzles.

From there, I’d like to amp this up a bit with scripts, sprites, and machine elves as power-ups.

The player can start by manually solving these network puzzles. But then, maybe they can purchase or “discover” scripts that automate some of the actions - e.g. when a security node is present, find its audit log collector and subvert it, repeat for all matches in the network. These should be pretty dumb and mechanical, just taking care of repetitive tasks for the player as a power-up. This can be another player progression path - accumulation of scripts makes tackling the repetitive parts of complex puzzles less tedious.

Sprites are semi-autonomous AIs. Kind of a Pokemon aspect I want to add to the game. They can do more than scripts. Send them into a network and they can automatically solve certain chunks of puzzles, leaving the player to resolve higher-level assemblies. Maybe some lower-complexity networks can be tackled entirely on automatic by certain sprites. This would imply puzzle designs that encode their own solutions and sprites can appear intelligent by “cheating” and accessing puzzle designs behind the scenes.

Machine elves are quasi-sentient AI weirdness entities. They aren’t generally possessed by the player, maybe more like alliances with loyalty missions. If a player convinces one to help, maybe they can just bulldoze through network in obscure ways that are not directly available to the player. Should seem like magic. Should backfire sometimes. Might imply that generated puzzles offer some weird backup alternative states to shift to, so that they at least make some sense within the context of the dungeon.

Would also be neat to release multiple sprites in parallel into a network. They could fan out to take down functional blocks of a network simultaneously.

Seems like time as a pressure factor would be fun in some dungeons, so time-saving techniques would be a fun power-up to mitigate that pressure.
