...
export function BaseNode({ rng, addr, childAddrs }) {
  return {
    addr,
    childAddrs,
    child: genChild(childAddrs, Galaxy),
  };
}

export const genChild = (childAddrs, childFn) => (addr) =>
  childAddrs.includes(addr) ? childFn({ addr }) : undefined;
